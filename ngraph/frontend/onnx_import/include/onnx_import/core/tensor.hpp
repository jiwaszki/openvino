...
namespace ngraph
{
    namespace onnx_import
    {
        ...
        public:
            ...
            std::shared_ptr<ngraph::op::Parameter> get_ng_constant() const
            {
                switch (m_tensor_proto->data_type())
                {
                case ONNX_NAMESPACE::TensorProto_DataType::TensorProto_DataType_BOOL:
        ...
        private:
            template <typename T>
            std::shared_ptr<ngraph::op::Parameter> make_ng_constant(const element::Type& type) const
            {
                auto constant =
                    std::make_shared<ngraph::op::Parameter>(type, m_shape, get_data<T>());
                if (m_tensor_proto->has_name())
                {
